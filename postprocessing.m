...
colorSolar = [0.20, 0.40, 0.65];  % Muted blue
colorSorp  = [0.85, 0.55, 0.25];  % Warm amber
colorBatt  = [0.15, 0.55, 0.45];  % Deep teal
colorGrid  = [0.65, 0.25, 0.25];  % Muted crimson

% Dark theme styling
figBg     = [0.08, 0.08, 0.08];    % Figure background
axBg      = [0.08, 0.08, 0.08];    % Axes background
textColor = [0.90, 0.90, 0.90];    % Text/axis colour
gridColor = [0.45, 0.45, 0.45];    % Grid colour
...
dE_sorption_kWh = ...
    (PsorptionCharge_kW - PsorptionDischarge_kW - PsorptionLoss_kW) ...
    .* dt_seconds / 3600;

E_sorption_graph_kWh = ...
    EStorageInitial / J_per_kWh + cumsum(dE_sorption_kWh);

E_sorption_graph_kWh = ...
    max(EStorageMin / J_per_kWh, ...
...
nexttile;
plot(tDays, PSupply, "LineWidth", 1.5, "Color", colorSolar); 
hold on;
...
nexttile;
plot(tDays, PSell, "LineWidth", 1.5, "Color", colorSolar); 
hold on;
...
applyDarkTheme(gcf, figBg, axBg, textColor, gridColor, fontName);
...
pct_Direct(validIdx) = ...
    (E_Direct_cum(validIdx) ./ E_Total_cum(validIdx)) * 100;

pct_Sorption(validIdx) = ...
    (E_Sorption_cum(validIdx) ./ E_Total_cum(validIdx)) * 100;

pct_Battery(validIdx) = ...
    (E_Battery_cum(validIdx) ./ E_Total_cum(validIdx)) * 100;

pct_Bought(validIdx) = ...
    (E_Bought_cum(validIdx) ./ E_Total_cum(validIdx)) * 100;

stackedMatrix = [pct_Direct, pct_Sorption, pct_Battery, pct_Bought];

figure('Name', 'Report: Stacked Energy Source Mix Timeline', ...
       'Color', figBg, ...
       'InvertHardcopy', 'off', ...
       'WindowStyle', 'docked');
...
ylabel('Cumulative Contribution Share [%]', ...
    'FontWeight', 'bold', ...
    'FontName', fontName, ...
    'Color', textColor);

title('Dynamic Delivered Energy Mix Evolution', ...
    'FontWeight', 'bold', ...
    'FontSize', 12, ...
    'FontName', fontName, ...
    'Color', textColor);
...
safePie(ax1, receivedValues, receivedLabels, customColors, figBg, textColor, fontName);

title(ax1, ...
    sprintf("Received Solar Infrastructure\nTotal Input: %.1f kWh", EfromSupplyTransport_kWh), ...
    "FontWeight", "bold", ...
    "FontName", fontName, ...
    "FontSize", 11, ...
    "Color", textColor);
...
safePie(ax2, deliveredValues, deliveredLabels, customColors, figBg, textColor, fontName);

title(ax2, ...
    sprintf("Demand-Side Consumption Mix\nTotal Pool: %.1f kWh", ...
    EDirect_kWh + ESorptionUseful_kWh + EBatteryUseful_kWh + EBuy_kWh), ...
    "FontWeight", "bold", ...
    "FontName", fontName, ...
    "FontSize", 11, ...
    "Color", textColor);
...
if ~all(isnan(EBattery))
    plot(tDays, EBattery / J_per_kWh, ...
        "LineWidth", 1.5, ...
        "Color", colorBatt); 
    hold on;

    yline(EBatteryMin / J_per_kWh, "--", ...
        "Battery min", ...
        'Color', colorGrid, ...
        'LineWidth', 1.2);

    yline(EBatteryMax / J_per_kWh, "--", ...
        "Battery max", ...
        'Color', colorSolar, ...
        'LineWidth', 1.2);
end
...
if ~all(isnan(EBattery))
    ylim([min(EBattery/J_per_kWh)*0.9 max(EBattery/J_per_kWh)*1.1]);
end

applyDarkTheme(gcf, figBg, axBg, textColor, gridColor, fontName);

%% ------------------------------------------------------------------------
%  8. Seven-day battery window
% -------------------------------------------------------------------------
...
grid on;
title("High-Resolution Battery Cycling Dynamics (7-Day Micro-Window)", ...
    "FontWeight", "bold");
...
nexttile;
plot(tDays, PfromSorptionStorage, ...
    "LineWidth", 1.5, ...
    "Color", colorSorp);
...
if ~all(isnan(ESorption))
    plot(tDays, ESorption / J_per_kWh, ...
        "LineWidth", 1.5, ...
        "Color", colorSorp); 
    hold on;

    yline(EStorageMin / J_per_kWh, "--", ...
        "Sorption min", ...
        'Color', colorGrid, ...
        'LineWidth', 1.2);

    yline(EStorageMax / J_per_kWh, "--", ...
        "Sorption max", ...
        'Color', colorSolar, ...
        'LineWidth', 1.2);
end
...
if ~all(isnan(ESorption))
    ylim([min(ESorption/J_per_kWh)*0.9 max(ESorption/J_per_kWh)*1.1]);
end
...
plot(tDays, E_sorption_graph_kWh, ...
    "LineWidth", 1.6, ...
    "Color", colorSorp);
...
function safePie(ax, values, labels, cmap, figBg, textColor, fontName)
    ...
    for k = 1:numel(pPatches)
        ...
        if isa(pPatches(k), 'matlab.graphics.primitive.Patch')
            ...
            pPatches(k).EdgeColor = textColor;
            pPatches(k).LineWidth = 1.2;

            styleIdx = styleIdx + 1;

        elseif isa(pPatches(k), 'matlab.graphics.shape.Text')

            pPatches(k).FontName = fontName;
            pPatches(k).FontSize = 9;
            pPatches(k).Color = textColor;
        end
    end

    legend(ax, labels(keep), ...
...
function applyDarkTheme(fig, figBg, axBg, textColor, gridColor, fontName)
    ...
    for i = 1:numel(axs)
        ...
        try
            axs(i).Title.Color = textColor;
            axs(i).XLabel.Color = textColor;
            axs(i).YLabel.Color = textColor;
            axs(i).ZLabel.Color = textColor;
        catch
        end
    end
    ...
    for i = 1:numel(allText)
        try
            allText(i).Color = textColor;
            allText(i).FontName = fontName;
        catch
        end
    end
